...
def main():
    ...
    config_path = (
        Path(sys.argv[1]) if len(sys.argv) > 1 else PROJECT_ROOT / "config" / "rgb_baseline.yaml"
    )
    ...
    ckpt_dir = PROJECT_ROOT / log_cfg.get("ckpt_dir", "checkpoints/rgb")
    results_dir = PROJECT_ROOT / log_cfg.get("results_dir", "results/rgb")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_epoch = epoch
            epochs_no_improve = 0
            torch.save(model.state_dict(), ckpt_dir / "rgb_baseline_best_seed{seed}.pt")
    ...
    best_ckpt = ckpt_dir / "rgb_baseline_best_seed{seed}.pt"
    ...
    json_path = results_dir / "rgb_baseline_results_seed{seed}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"[INFO] Saved JSON results to {json_path}")

    # -------------------- Save confusion matrix heatmap ---
    cm_path = results_dir / "rgb_confusion_matrix_seed{seed}.png"
    plot_confusion_matrix(test_cm, class_names, cm_path, title="RGB Baseline Confusion Matrix")
