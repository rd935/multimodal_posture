...
from models.rgb_depth_baselines import DepthBaselineResNet18
...
def main():
    ...
    # Load YAML config
    # -----------------------------------------------------
    # Default config path: config/depth_baseline.yaml
    config_path = (
        Path(sys.argv[1]) if len(sys.argv) > 1 else PROJECT_ROOT / "config" / "depth_baseline.yaml"
    )
    ...
    rgb_frames = int(data_cfg.get("rgb_frames", 16))  # frames used to align depth
    resize = tuple(data_cfg.get("resize", [224, 224]))
    num_workers = int(loader_cfg.get("num_workers", 4))

    ckpt_dir = PROJECT_ROOT / log_cfg.get("ckpt_dir", "checkpoints/depth")
    results_dir = PROJECT_ROOT / log_cfg.get("results_dir", "results/depth")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            best_epoch = epoch
            epochs_no_improve = 0
            torch.save(model.state_dict(), ckpt_dir / "depth_baseline_best.pt")
    ...
    best_ckpt = ckpt_dir / "depth_baseline_best.pt"
    ...
    json_path = results_dir / "depth_baseline_results.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"[INFO] Saved JSON results to {json_path}")

    cm_path = results_dir / "depth_confusion_matrix.png"
